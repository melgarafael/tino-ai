...
//   - thumbs_down match : -1.5 por token de titulo thumbed-down (cap -3.0)
//   - recencia (<=7 dias): +1.0
...
// Tokeniza titulo de item thumbed-down em palavras >= 4 chars (filtro de stopwords basico).
// Retorna Set de tokens lowercase.
const STOPWORDS = new Set([
  'the', 'and', 'for', 'with', 'from', 'into', 'that', 'this', 'your', 'have',
  'will', 'about', 'their', 'which', 'would', 'could', 'should', 'como', 'para',
  'pelo', 'pela', 'mais', 'menos', 'essa', 'esse', 'isso', 'esta', 'este',
  'sobre', 'entre', 'quando', 'porque',
]);

function collectThumbsDownTokens(ajustes) {
  const tokens = new Set();
  const list = Array.isArray(ajustes) ? ajustes : (ajustes ? [ajustes] : []);
  for (const a of list) {
    if (!a) continue;
    const thumbs = a.thumbs_down;
    if (!Array.isArray(thumbs)) continue;
    for (const entry of thumbs) {
      if (!entry) continue;
      const title = typeof entry === 'string' ? entry : String(entry.titulo || '');
      if (!title) continue;
      const parts = title.toLowerCase().split(/[^\p{L}\p{N}_]+/u);
...
export function rankMock(perfil, novidade, ajustes = []) {
  ...
  const thumbsDownTokens = collectThumbsDownTokens(ajustes);
  ...
  const thumbTokenMatches = thumbsDownTokens.filter((t) => hasTerm(hay, t));
  ...
  const thumbPenalty = Math.min(thumbTokenMatches.length * 1.5, 3.0);
  const recencyBonus = isRecent(novidade?.data) ? 1.0 : 0.0;

  const raw = 5.0 + focoBonus + identBonus + recencyBonus - evitaPenalty - tagPenalty - thumbPenalty;
  ...
  if (thumbTokenMatches.length) motivos.push(`thumbs_down tokens: ${thumbTokenMatches.join(', ')}`);
  if (recencyBonus) motivos.push('recencia (<=7d)');

  const justificativa = motivos.length
    ? `Match heuristico no seu perfil — ${motivos.join('; ')}.`
    : 'Sem matches diretos no perfil — score neutro pelo ranker heuristico (mock).';

  // ---- Schema rico (v2) — placeholders honestos do mock heurístico ----
  // O mock não inventa narrativa: emite campos coerentes e marcados como (mock).
  // Quando o ranker real (rank-claude) processa, ele sobrescreve com qualidade alta.
  const tldr = veredito === 'Foca'
    ? `Tem match com seu foco: ${focoMatches.join(', ') || 'tópico relevante'} — vale revisar.`
    : veredito === 'Considera'
      ? `Toca temas do seu perfil (${(identMatches[0] || focoMatches[0] || 'identidade')}) — relevância contextual.`
      : veredito === 'Acompanha'
        ? 'Tangencia o que você acompanha — sem urgência.'
        : 'Fora do seu contexto atual — pode dispensar.';

  const next_step = veredito === 'Foca'
    ? 'Ler nesta semana e considerar agir em cima.'
    : veredito === 'Considera'
      ? 'Agendar leitura quando puder; não é urgente.'
      : veredito === 'Acompanha'
        ? 'Marcar mentalmente; revisitar se virar foco.'
        : 'Dispensar — fora do escopo.';

  const tradeoff = '';

  // Confiança 0-5 baseada em força dos sinais.
  const confidence = Math.max(0, Math.min(5, focoMatches.length * 2 + identMatches.length + (recencyBonus ? 1 : 0) - evitaMatches.length));

  // Tempo de leitura estimado pelo tamanho do resumo (heurística grosseira).
  const read_min = Math.max(2, Math.min(10, Math.ceil(String(resumoOut).length / 280)));

  // Ancoragens — só o que o mock realmente sabe: termos do perfil que casaram.
  const anchors = [];
  if (focoMatches.length) anchors.push(`_perfil.md · foco_ativo | ${focoMatches.join(', ')}`);
  if (identMatches.length) anchors.push(`_perfil.md · identidade | ${identMatches.join(', ')}`);
  if (evitaMatches.length) anchors.push(`_perfil.md · evita | ${evitaMatches.join(', ')}`);
